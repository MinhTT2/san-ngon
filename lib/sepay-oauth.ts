...
export async function requireSepayOwner(request?: NextRequest) {
  ...
  if (!data || !['owner', 'admin'].includes(data.role) || data.owner_application_status !== 'active') throw new Error('OWNER_NOT_APPROVED');
  return user.id;
}
...
const ERRORS: Record<string, string> = {
  AUTH_REQUIRED: 'Bạn cần đăng nhập lại.', OWNER_NOT_APPROVED: 'Tài khoản chủ sân chưa được duyệt.',
