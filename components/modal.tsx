...
export function Modal({ title, subtitle, onClose, children, size = 'max-w-3xl' }: { title: string; subtitle?: string; onClose: () => void; children: React.ReactNode; size?: string }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6" role="dialog" aria-modal="true" aria-labelledby="modal-title">
    <button type="button" aria-label="Đóng cửa sổ" onClick={onClose} className="absolute inset-0 h-full w-full cursor-default bg-pitch/35" />
    <section className={`relative flex max-h-[calc(100dvh-1.5rem)] w-full ${size} flex-col overflow-hidden rounded-card border border-hairline bg-card sm:max-h-[calc(100dvh-3rem)]`}>
