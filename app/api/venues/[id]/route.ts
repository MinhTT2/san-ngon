...
import { VenueManagementBody, validationFieldErrors, rpcFieldErrors } from '@/lib/owner-management';
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error) return NextResponse.json({ error: venueErrorMessage(error.message), fieldErrors: rpcFieldErrors(error.message, venueErrorMessage(error.message)) }, { status: 400 });
