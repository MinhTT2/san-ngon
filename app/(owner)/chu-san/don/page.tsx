...
export default async function OwnerBookingsPage({ searchParams }: { searchParams: Promise<{ venue?: string; status?: string; date?: string }> }) {
  ...
  return <main className="mx-auto max-w-7xl px-5 py-8 lg:px-10 lg:py-10">
    ...
    {venues.length > 1 && <nav aria-label="Chọn cụm sân" className="mt-7 flex gap-2 overflow-x-auto pb-1">{venues.map((item) => <Link key={item.id} href={`/chu-san/don?venue=${item.id}&status=${status}${selectedDate ? `&date=${selectedDate}` : ''}`} className={`shrink-0 rounded-control border px-4 py-2.5 text-sm font-semibold ${item.id === venue.id ? 'border-pitch bg-pitch text-pitch-ink' : 'border-hairline bg-card text-ink-secondary hover:border-strong'}`}>{item.name}</Link>)}</nav>}
