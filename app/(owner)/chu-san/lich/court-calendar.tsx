...
export function CourtCalendar({ courtId }: { courtId: string }) {
  ...
  async function close() {
    ...
    setBusy(false);
    if (!response.ok) { setError(result.error ?? 'Không khóa được lịch.'); return; }
    setReason(''); setStart(''); setEnd(''); await load();
  }
  ...
  return <div className="mt-8 space-y-6">
    ...
    <section className="rounded-card border border-hairline bg-card p-4 lg:p-5">
      ...
      <div className="mt-5 border-t border-hairline pt-5"><h3 className="font-semibold">Khóa lịch</h3><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><label className="flex items-center gap-2 text-sm font-medium sm:col-span-2 lg:col-span-4"><input type="checkbox" checked={partial} onChange={(e) => setPartial(e.target.checked)} /> Chỉ khóa một khoảng giờ</label>{partial && <><label className="text-sm font-semibold">Từ<input className={`${OWNER_INPUT} mt-1`} type="time" value={start} onChange={(e) => setStart(e.target.value)} /></label><label className="text-sm font-semibold">Đến<input className={`${OWNER_INPUT} mt-1`} type="time" value={end} onChange={(e) => setEnd(e.target.value)} /></label></>}<label className="text-sm font-semibold sm:col-span-2">Lý do (không bắt buộc)<input className={`${OWNER_INPUT} mt-1`} value={reason} maxLength={200} onChange={(e) => setReason(e.target.value)} placeholder="Bảo trì, giải đấu..." /></label></div><button type="button" disabled={busy} onClick={close} className={`${OWNER_PRIMARY} mt-4`}>{partial ? 'Khóa khoảng giờ' : 'Khóa cả ngày'}</button></div>
