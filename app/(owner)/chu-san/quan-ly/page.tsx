...
export default async function Page({ searchParams }: { searchParams: Promise<{ venue?: string }> }) {
  ...
  const { data: profile } = await supabase.from('profiles').select('role, owner_application_status').eq('id', user.id).maybeSingle();
  ...
  return <main className="mx-auto max-w-7xl px-5 py-10 lg:px-16"><header className="flex flex-wrap items-start justify-between gap-5"><div className="max-w-2xl"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-pitch">Quản lý vận hành</p><h1 className="mt-3 font-display text-4xl font-extrabold tracking-tight text-pitch">Cụm sân và sân con.</h1><p className="mt-4 text-[15px] leading-7 text-ink-secondary">Sửa thông tin, tắt sân tạm thời hoặc thêm sân mới. Đơn đã phát sinh sẽ không bị xóa nhầm.</p></div><Link href="/tao-cum-san" className="inline-flex min-h-12 items-center justify-center rounded-control bg-pitch px-5 py-3 text-sm font-semibold text-pitch-ink">+ Thêm cụm sân</Link></header><div className="mt-8"><VenueManagement initialVenues={(data ?? []) as unknown as Parameters<typeof VenueManagement>[0]['initialVenues']} selectedVenueId={selectedVenueId} /></div></main>;
}
